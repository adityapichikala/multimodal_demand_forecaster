...
is_dummy_gemini = (
    not gemini_key or "your_gemini" in gemini_key.lower() or gemini_key == ""
)
is_dummy_openrouter = (
    not openrouter_key
    or "your_openrouter" in openrouter_key.lower()
    or openrouter_key == ""
)

# Provide valid dummy key strings for initialization to prevent Pydantic validation errors during import.
gemini_init_key = gemini_key if not is_dummy_gemini else "dummy_key_for_initialization"
openrouter_init_key = (
    openrouter_key if not is_dummy_openrouter else "dummy_key_for_initialization"
)
