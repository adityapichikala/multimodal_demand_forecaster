...
def run_async_forecast(self, store_id: int, product_id: int):
    """
    ...
    try:
        ...
        df = pd.DataFrame([{"date": s.date, "store": s.store_id, "item": s.product_id, "sales": s.sales} for s in sales])
        df['date'] = pd.to_datetime(df['date'])
        
        # Run Prophet
        result = run_forecast(df, store=store_id, item=product_id, periods=7)
