...
async def login_for_access_token(
    ...
    if not merchant or not verify_password(
        form_data.password, merchant.hashed_password
    ):
        raise HTTPException(
            status_code=401,
...
async def get_dashboard_meta(
    ...
    products = (
        db.query(Product.item_id, Product.name)
        .join(HistoricalSale, Product.id == HistoricalSale.product_id)
    ...
    stores = (
        db.query(HistoricalSale.store_id)
...
async def get_forecast_history(
    ...
    forecasts = (
        ...
        .join(models.Product)
        .filter(models.Product.merchant_id == current_merchant.id)
    ...
    try:
        if clear_all:
            ...
            db.query(models.Forecast).filter(
                models.Forecast.product_id.in_(
            ...
            db.query(models.HistoricalSale).filter(
                models.HistoricalSale.product_id.in_(
                    db.query(models.Product.id).filter(
                        models.Product.merchant_id == current_merchant.id
                    )
                )
        ...
        for p_id_raw, row in unique_products.iterrows():
            ...
            if not prod:
                prod = Product(
                    item_id=p_id_str, name=p_name, merchant_id=current_merchant.id
                )
                db.add(prod)
                db.commit()
                db.refresh(prod)
...
async def train_async(
    ...
    product = (
        db.query(Product)
        .filter(Product.item_id == item, Product.merchant_id == current_merchant.id)
        .first()
    )
    if not product:
        raise HTTPException(
            status_code=404,
            detail="Product not found. Please upload historical data for this item first.",
        )

    task = run_async_forecast.delay(store_id=store, product_pk=product.id)
    return JSONResponse(
        status_code=202, content={"task_id": task.id, "status": "Processing"}
    )
...
async def analyze(
    ...
    try:
        report = run_verification_pipeline(
            forecast_summary=summary, weather_text=weather_text, news_text=news_text
        )
    except Exception as e:
        if (
            "429" in str(e)
